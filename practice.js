...
const each = (names, cb) => {
  names.forEach((name, i) => cb(name, i))
}
...
const startWithLetterA = (things) => things.filter((str) => str[0].toLowerCase() === 'a')
...
const findProgrammer = (employees) => {
  const programmer = employees.filter((employee) => employee.job === 'programmer')
  return programmer[0]
}
...
const exampleMenuItems = [
  ...
  { item: 'Grilled chicken', price: 18 },
  { item: 'Pasta primavera', price: 15 },
];
// Do not edit code above.
